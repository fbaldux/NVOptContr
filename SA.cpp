/*  -------------------------------------------------------------------------------------------  //
    ...
    - The configurations found are saved to Configurations/s\_{...}, with the # of pulses and 1/eta
      in the header.
...
int main( int argc, char *argv[] ) {
    ...
    if( argc != 11 ) {
        cerr << "\nError! Usage: ./SA <Tfin> <Delta_t> <tone> <harmonic> <ann_steps> <MC_steps> <Temp0> <K> <reps_sig> <reps_each>\n\n";
        exit(-1);
    }    
    ...
    int rep_sig = strtod(argv[9], NULL);
    int reps_each = strtod(argv[10], NULL);
    ...
    // create the output file
    char filename[100];
    snprintf(filename, 100, "Results/SA_T%.4f_dt%.4f_t%d_h%d_K%.4f_r%d.txt", Tfin, Delta_t, tone, harmonic, K0, rep_sig);        
