/*  -------------------------------------------------------------------------------------------  //
    ...
    - The initial configuration is given by the zeros of the signal, i.e. the generalized CP
      sequence (it is generated by the spherical\_{...} file).
    - The only allowed MC moves are domain wall shifts.
    - The energy is computed efficiently at each step.
    - The configurations found are saved to Configurations/sGCPAnn_\{...}, with the # of pulses and 1/eta
      in the header.
...
void load_s(int *s, int rs) {
    char filename[100];
    snprintf(filename, 100, "Configurations/sGCP_T%.4f_dt%.4f_t%d_h%d_r%d.txt", Tfin, Delta_t, tone, harmonic, rs);         
    ifstream infile(filename);
...
void save_s(int *s, int dw, double this_etaInv, int r) {
    ...
    char filename[100];
    snprintf(filename, 100, "Configurations/sGCPAnn_T%.4f_dt%.4f_t%d_h%d_r%d.txt", Tfin, Delta_t, tone, harmonic, r);        
